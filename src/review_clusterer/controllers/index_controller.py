...
def index_controller(csv_file_path: Path, use_local_embedder: bool = False) -> None:
    ...
    try:
        ...
        if use_local_embedder:
            console.print("[2/3] Creating embeddings with local sentence-transformers model...", style="bold")
            embedder = LocalEmbedder()
        else:
            console.print("[2/3] Creating embeddings with VoyageAI...", style="bold")
            embedder = VoyageEmbedder(api_key="pa-ZJzGbg--jB3Nq3dRz0cRPAhdLhCGzWeI1DNLxQfhnMP")
        ...
        console.print(f"  [green]✓[/green] Created embeddings for {review_count} reviews")
        ...
        if db_directory.exists():
            console.print(f"  [yellow]![/yellow] Found existing database at {db_directory}")
            console.print(f"  [yellow]![/yellow] Deleting existing database...")
        ...
        console.print(f"  [green]✓[/green] Saved {review_count} reviews to ChromaDB at {db_directory}")
